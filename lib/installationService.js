...
class installationService {
	...
	async getInstallationByUserUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate, iD.endDate from installation i
	...
	*/
	async getInstallationByUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state,
										i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate as deviceStartDate, iD.endDate as deviceEndDate, iU.userUUID as sentiUserUUID,
	...
	async getInstallationByID(id) {
		if (id) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, iU.uuid as instUserUUID, iU.userUUID as sentiUserUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate, iD.endDate
							from installation i
							LEFT JOIN instDevice iD on i.uuid = iD.instUUID
							LEFT JOIN instUser iU on i.uuid = iU.instUUID
	...
	async getInstallationsByOrgUUID(orgUUID) {
		if (orgUUID) {
			let selectSQL = `SELECT i.uuid, iD.uuid AS instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state,
										i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate AS deviceStartDate, iD.endDate AS deviceEndDate, iU.userUUID AS userUUID,
										iU.startDate AS userStartDate, iU.endDate AS userEndDate, iU.adults, iU.children
								FROM installation i
									LEFT JOIN instDevice iD on i.uuid = iD.instUUID AND iD.startDate <= NOW() AND (ISNULL(iD.endDate) OR iD.endDate >= NOW())
									LEFT JOIN instUser iU on i.uuid = iU.instUUID AND iU.startDate <= NOW() AND (ISNULL(iU.endDate) OR iU.endDate >= NOW())
	...
	async getInstallationsByUserUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate, iD.endDate, iU.userUUID as userUUID, iU.adults, iU.children from installation i
							LEFT JOIN instDevice iD on i.uuid = iD.instUUID
							LEFT JOIN instUser iU on i.uuid = iU.instUUID
							where iU.userUUID=? and i.deleted=0`
			let selectQuery = await this.db.query(selectSQL, [uuid])
	 ...
	 * Get all installations
	 */
	async getInstallationsByUUIDs(uuids) {
		if (uuids.length) {
			let selectSQL = 'SELECT i.uuid, iD.uuid AS instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.`long`, i.orgUUID, i.state, ' +
								'		i.operation, i.moving, i.installationIdent, iD.deviceUUID, iD.startDate AS deviceStartDate, iD.endDate AS deviceEndDate, iU.userUUID AS userUUID, ' +
								'		iU.startDate AS userStartDate, iU.endDate AS userEndDate, iU.adults, iU.children ' +
								...
								'	LEFT JOIN instDevice iD on i.uuid = iD.instUUID AND iD.startDate <= NOW() AND (ISNULL(iD.endDate) OR iD.endDate >= NOW()) ' +
								'	LEFT JOIN instUser iU on i.uuid = iU.instUUID AND iU.startDate <= NOW() AND (ISNULL(iU.endDate) OR iU.endDate >= NOW()) ' +
								'WHERE iD.deviceUUID IN(?' + ",?".repeat(uuids.length - 1) + ') AND i.deleted=0 ' +
								'GROUP BY i.uuid'
			let selectQuery = await this.db.query(selectSQL, [...uuids])
