...
class installationService {
	...
	constructor(auth) {
		this.db = mysqlConn
		this.dbb = databrokerAPI
		this.dbb.setHeader('Authorization', auth)
		console.log(this.dbb.headers)
	}
	...
	*/
	async getInstallationByUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT * from installation where uuid=? and deleted=0`
	...
	async getInstallationByID(id) {
		if (id) {
			let selectSQL = `SELECT * from installation where id=? and deleted=0`
	...
	async createInstallation(installation) {
		if (installation) {
			...
			inst.uuid = uuidv4()
			let insertSQL = `INSERT INTO installation (uuid, address, orgUUID, state, adults, children, deleted) VALUES(?, ?, ?, ?, ?, ?, 0);`
