...
class installationService {
	...
	async getInstallationByUserUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, i.streetName, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, iD.deviceUUID, iD.startDate, iD.endDate from installation i
	...
	*/
	async getInstallationByUUID(uuid) {
		if (uuid) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, iU.uuid as instUserUUID, iU.userUUID as sentiUserUUID, i.streetName, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, iD.deviceUUID, iD.startDate, iD.endDate
							from installation i
							LEFT JOIN instDevice iD on i.uuid = iD.instUUID
							LEFT JOIN instUser iU on i.uuid = iU.instUUID
	...
	async createInstallation(installation) {
		if (installation) {
			...
			inst.uuid = uuidv4()
			let insertSQL = `INSERT INTO installation (uuid, streetName, lat, \`long\`, orgUUID, state, moving, operation, deleted) VALUES(?, ?, ?, ?, ?, ?, ?, ?, 0);`
			let insertQuery = await this.db.query(insertSQL, [inst.uuid, inst.streetName, inst.lat, inst.long, inst.orgUUID, inst.state, inst.moving, inst.operation])
		...
		if (installation) {
			...
			if (inst) {
				let updateSQL = `UPDATE installation
								SET operation=?,streetName=?,state=?,moving=?,orgUUID=?,lat=?,\`long\`=?
								WHERE uuid=?;`
				let i = installation
				let updateQuer = await this.db.format(updateSQL, [i.operation, i.streetName, i.state, i.moving, i.orgUUID,i.lat, i.long, i.uuid])
				console.log(updateQuer)
				let updateQuery = await this.db.query(updateSQL, [i.operation, i.streetName, i.state, i.moving, i.orgUUID,i.lat, i.long, i.uuid])
