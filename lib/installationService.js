...
class installationService {
	...
	*/
	...
	async getInstallationsByOrgUUID(orgUUID) {
		if (orgUUID) {
			let selectSQL = `SELECT i.uuid, iD.uuid as instDevUUID, i.streetName, i.streetNumber, i.side, i.city, i.zip, i.lat, i.\`long\`, i.orgUUID, i.state, i.operation, i.moving, iD.deviceUUID, iD.startDate, iD.endDate, iU.uuid as userUUID from installation i
							LEFT JOIN instDevice iD on i.uuid = iD.instUUID
							LEFT JOIN instUser iU on i.uuid = iU.instUUID
							where orgUUID=? and i.deleted=0;`
			let selectQuery = await this.db.query(selectSQL, [orgUUID])
			if (selectQuery[0].length > 0) {
				let userUUIDs = selectQuery[0].map(i => i.userUUID)
				let getUsers = await this.core.post('/v2/entity/waterworks/users', {uuids: userUUIDs})
				console.log("Get Users", getUsers.length)
				let fInsts = selectQuery[0].map(i => {
					let i = new Installation(i)
					i.user = getUsers[getUsers.findIndex(f => f.uuid === i.userUUID)]
					return i
				})
				return fInsts
			}
