	...
	async createInstUser(instUser) {
		if (instUser) {
			...
			let insertSQL = `INSERT INTO instUser
							...
							VALUES(?, ?, ?, ?, ?, ?, ?, 0);`
			let insertQuery = await this.db.query(insertSQL, [inst.uuid, inst.startDate, inst.endDate, inst.userUUID, inst.adults, inst.children, inst.instUUID])
		...
		if (instUser) {
			...
			if (inst) {
				let updateSQL = `UPDATE instUser
						SET startDate=?, endDate=?, userUUID=?, instUUID=?, adults=?, children=?, deleted=0
						WHERE uuid=?;`
				let i = inst
				let updateQuery = await this.db.query(updateSQL, [i.startDate, i.endDate, i.userUUID, i.instUUID, i.adults, i.children, i.uuid])
